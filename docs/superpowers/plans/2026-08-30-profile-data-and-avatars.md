...
- [ ] **Step 1: Write the failing validation tests** — in `backend/src/validation.test.ts`, replace the age/sexAtBirth cases with:
...
- [ ] **Step 2: Run to verify failure** — `npx vitest run backend/src/validation.test.ts` — expect FAIL (`GENDER_OPTIONS` not exported, gender not validated).

- [ ] **Step 3: Implement** — in `backend/src/db/schema.ts`: delete `SEX_AT_BIRTH_OPTIONS`/`SexAtBirth`, add:
...
- [ ] **Step 4: Verify green** — `npm run typecheck && npm test` — expect PASS, all suites.

- [ ] **Step 5: Generate the migration** — `DATABASE_URL=postgresql://x:y@localhost:5432/gen npm run db:generate`, rename the file to `drizzle/0005_single_gender_field.sql`, update the `tag` in `drizzle/meta/_journal.json` to match, then prepend the data fix so the drop and the enum-tightening land together:
...
- [ ] **Step 1: Write the failing screen test** — `frontend/__tests__/signup.test.tsx`:
...
- [ ] **Step 4: Verify green** — `cd frontend && npx tsc --noEmit && npm test` — expect PASS (fix any other test fixtures that referenced the removed fields).

- [ ] **Step 5: Commit** — `git add -A && git commit -m "Ask for gender and the SHPE member ID at signup"`.
...
- [ ] **Step 2: Grant the runtime SA its two roles** — append to `infra/iam.tf`:
...
- [ ] **Step 1: Write the failing route tests** — `backend/src/routes/profile.test.ts`, following the app-boot + `vi.mock` pattern of `backend/src/routes/auth.test.ts` (mock `../auth/firebase` for `requireAuth`'s `verifyIdToken`, mock `../db` for the user lookup/update chains, and mock `../avatars/storage`):
...
- [ ] **Step 2: Run to verify failure** — `npx vitest run backend/src/routes/profile.test.ts` — expect FAIL (module missing).

- [ ] **Step 3: Implement** — `backend/src/avatars/storage.ts`:
...
- [ ] **Step 4: Generate migration 0006** — as Task 1 Step 5 (`0006_add_avatar_path.sql`, journal tag to match); it should contain only `ALTER TABLE "users" ADD COLUMN "avatar_path" text;`.

- [ ] **Step 5: Verify green** — `npm run typecheck && npm test` — expect PASS.

- [ ] **Step 6: Commit** — `git add -A && git commit -m "Issue signed avatar uploads and record the chosen object"`.
...
- [ ] **Step 1: Write the failing component test** — `frontend/components/Avatar.test.tsx`:
...
- [ ] **Step 5: Verify green** — `cd frontend && npx tsc --noEmit && npm test` — expect PASS.

- [ ] **Step 6: Commit** — `git add -A && git commit -m "Let members set a profile picture, shown wherever members appear"`.
