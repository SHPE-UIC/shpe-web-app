...
**Targeting a subset of the chapter** — the reason `users.majors` exists — is not built by this plan, but the column it would use is: `where majors && array[...]` selects members by programme. `users.major_other` is deliberately outside that array and must never be sent to, however the feature is eventually shaped. See `docs/superpowers/plans/2026-09-02-school-year-majors-and-uin.md`.
